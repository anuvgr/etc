...
const SalesReturns = () => {
  ...
  return (
    <div>
          ...
          <tbody>
            {filteredReturns.map(ret => (
              <tr key={ret.id}>
                ...
                <td>
                  <div>{ret.company_name || ret.customer_name}</div>
                  {ret.company_name && <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{ret.customer_name}</div>}
                </td>
                ...
                <td>
                  <button onClick={() => window.open(`/print/sales-return/${ret.id}`, '_blank')} className="btn btn-outline">
      ...
      {showForm && (
        <SalesReturnForm onCancel={() => setShowForm(false)} onSave={() => { setShowForm(false); fetchReturns(); }} />
      )}
    </div>
  );
};
